...
**Thus, users are not limited to evaluating the SiGN model—this function can be applied to any model that produces predictions on the (0, 1) interval, such as logistic regression, reinforcement learning models, or other proportion-based models.** See [Application to Other Models](#application-to-other-models) at the bottom of this article.
...
While `choice_mod_eval()` was designed with the SiGN model in mind, it is not restricted to that use case. The function can be applied to any model that produces predicted values on the (0, 1) interval—such as logistic regression models, reinforcement learning models, or any framework predicting choice or response proportions.

However, several important caveats should be considered before applying it more broadly:

⚠️ Important Caveats: 

You must specify the number of free parameters used by the predictive model via the `k` argument. For example, if the model used to generate the predictions includes three free parameters, call the function like this:

```r
choice_mod_eval(observed, predicted, k = 3)
```

The function will automatically add one additional parameter to account for the precision term ($\phi$) used in the Beta error model when calculating AIC and BIC.

When using `choice_mod_eval()` and BIC-derived Bayes Factors, keep the following assumptions and limitations in mind:

* **The predictive model should specify only the mean structure.**
That is, the model should output **expected values** (e.g., choice proportions) without incorporating its own error distribution or stochastic simulation. If the model already generates variability (e.g., by sampling outcomes or defining its own likelihood), then applying an additional Beta error model on top is likely inappropriate, as this violates the assumptions behind `choice_mod_eval()`.

* **BIC provides only an approximation of the true Bayes Factor.**
As such, this method performs best with a moderate to large sample size. A common rule of thumb is at least 30 observations for reliable model comparison. With fewer than 30 data points, BIC-based Bayes Factors may be unstable or misleading.

* **The sample size should be large relative to the number of free parameters.**
Specifically, the ratio of observations ($n$) to free parameters ($k$) should be much greater than 1. A good minimum is $n/k > 10$; for nonlinear models, $n/k > 20$ is often preferable. If $n/k \approx 1$, the model is likely overfitting—capturing noise instead of the underlying signal—and may not generalise well.

* **Observations should be independent.**
This is critical for likelihood-based comparisons. When fitting data from individual subjects across multiple conditions, ensure that condition order is randomised and that proportions are aggregated across many trials (ideally after a learning or burn-in phase). Trial-level binary choices are often autocorrelated and may violate the independence assumption.
